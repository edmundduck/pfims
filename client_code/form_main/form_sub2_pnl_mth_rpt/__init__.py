...
class form_sub2_pnl_mth_rpt(form_sub2_pnl_mth_rptTemplate):
  ...
  def __init__(self, **properties):
    ...
    self.dropdown_time.items = self.interval_list
    ...
    if self.dropdown_time.selected_value != "SDR":
  ...
  def dropdown_time_change(self, **event_args):
    ...
    if self.dropdown_time.selected_value != "SDR":
      ...
      self.tranx_rpt_time_to_date.enabled = False
      self.tranx_rpt_time_to_label.enabled = False
      self.dropdown_symbol.items = self.symbol_list + anvil.server.call('get_symbol_dropdown_items', 
                                                               date.today(), 
                                                               anvil.server.call('get_start_date', 
                                                                                 date.today(), 
                                                                                 self.dropdown_time.selected_value))
    else:
  ...
  def tranx_rpt_button_plus_click(self, **event_args):
    ...
    if self.tag['added_symbols'].get(self.dropdown_symbol.selected_value, None) is None:
      b = Button(text=self.dropdown_symbol.selected_value,
                ...
                foreground="White",
                background="Blue")
      self.symbol_panel.add_component(b, name=self.dropdown_symbol.selected_value)
