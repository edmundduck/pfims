...
@anvil.server.callable
# DB table "temp_template" update/insert method with time handling logic
...
def get_input_templ_name(templ_choice_str):
  if templ_choice_str is None or templ_choice_str == DEFAULT_NEW_TEMPL_TEXT:
    ...
    row = app_tables.temp_template.get(template_id=split_templ_id(templ_choice_str))
...
def get_input_templ_list():
  content = list(merge_templ_id_name(row['template_id'], row['template_name']) for row in app_tables.temp_template.search())
